...
/* =========================================================
   COMPLETE BACKUP
   Everything Including custom audio
========================================================= */
...
/*
 * DAILY IMPROVEMENT LIMIT
 *
 * Maximum 10 successful mission completions
 * per IST calendar day.
 *
 * Marketplace spending and mission penalties
...
if (!missionHistory[purchaseDate]) {
    missionHistory[purchaseDate] = {
        completed: 0,
        missed: 0,
        pointsDelta: 0,
        marketplaceSpent: 0,
...
missionHistory[purchaseDate].marketplaceSpent =
    (
        Number(
            missionHistory[purchaseDate]
                .marketplaceSpent
...
function pushNotification(title, msg) {
    ...
    appNotifications.unshift({
        ...
        time: getISTDate().toLocaleString(
            "en-IN",
            {
                dateStyle: "medium",
                timeStyle: "short"
...
function getMissionIdentity(li) {
    if (!li) {
        return "unknown";
    }
...
function recordMissionPerformance(
    ...
    /*
     * Allow a legitimate transition:
     *
     * missed → completed
     *
     * This happens when a user completes a
     * mission after its deadline was already
     * recorded as missed.
     */
    if (
        existingEvent.status === "missed" &&
        status === "completed"
    ) {

        /*
         * Remove the old missed count.
         */
        if (
            missionHistory[date].missed > 0
        ) {
            missionHistory[date].missed--;
        }

        /*
         * Add the completed count.
         */
        missionHistory[date].completed++;

        /*
         * Replace the old missed status.
         */
        existingEvent.status =
            "completed";

        /*
         * The old missed event already
         * contains its penalty.
         *
         * Add the completion reward on top
         * of the existing historical delta.
         */
        existingEvent.pointsDelta =
            Number(existingEvent.pointsDelta) +
            Number(pointsDelta);
        ...
        /*
         * Recalculate the day's total from
         * its events so the historical number
         * cannot drift.
         */
        missionHistory[date].pointsDelta =
            missionHistory[date].events.reduce(
                (total, event) =>
                    total +
    ...
    /*
     * Any other duplicate occurrence should
     * still be ignored.
     */
    return;
}
...
function calculateMonthMetrics(year, month) {
    ...
    days.forEach(date => {
        ...
        metrics.missionPoints +=
    Number(day.pointsDelta) || 0;

metrics.marketplaceSpent +=
    Number(day.marketplaceSpent) || 0;            
...
function startStaticTimer(h, m, s) {
    ...
    // Actual timing is based on Date.now(),
    // so interval throttling cannot cause drift.
    timerInterval =
        setInterval(
            updateTimer,
            250
...
function addMission() {
    ...
    li.dataset.repeatKey = getRepeatKey(null, repeat)
...
function updateMission() {
    ...
    */
    ...
    getRepeatKey(null, newRepeat);

li.dataset.repeatKey =
    currentRepeatKey;

/*
 * Preserve completion when recurrence changes.
 *
 * A Daily → Weekly or Weekly → Monthly change
 * uses a different repeatKey, so check today's
 * mission history rather than only the new key.
 */
const missionId =
    li.dataset.missionId;
...
const alreadyCompletedToday =
    Object.values(missionHistory).some(day => {
...
if (
    ...
    /*
     * Record Hardcore failure in mission history.
     *
     * This is important because the monthly report
     * must know that the Hardcore mission was missed
     * and that it caused a negative IP change.
     */
...
function getRepeatKey(date = null, repeat = "daily") {

    const d =
        date === null
            ? getISTDate()
            : getISTDate(new Date(date));
...
function getOccurrenceDateFromKey(
    key,
    repeat
) {
     ...
     * WEEKLY
     *
     * repeatKey represents Sunday of
     * that week.
     */
    if (repeat === "weekly") {
...
function refreshRecurringMissions() {
    ...
    missions.forEach(li => {
         ...
         * -------------------------------------------------
         *
         * Process every occurrence between the
         * previous occurrence and today.
         */
        let occurrenceDate =
            getOccurrenceDateFromKey(
                previousKey,
                repeat
            );
        ...
        /*
         * Move forward one recurrence at a time.
         */
        while (
            getRepeatKey(
                occurrenceDate,
            ...
            /*
             * Only process an occurrence that
             * has actually passed.
             */
            if (
                missedKey !== currentKey
            ) {
...
const alreadyRecorded =
        ...
        event =>
            event.key === occurrenceKey
                ...
                if (
                    ...
                    /*
                     * Deduct one IP for a missed
                     * normal occurrence.
                     *
                     * Negative IP is intentional.
                     */
                    const pointsBefore =
                        completedMissions;
    ...
    completedMissions - 1
        ...
        /*
         * Finally activate today's occurrence.
         */
        resetRecurringMission(
            li,
            currentKey
...
function resetRecurringMission(li, currentKey) {
    ...
    li.dataset.completed =
        "false";
         ...
         /*
     * Move the deadline to the current
    ...
    if (li.dataset.deadline) {
        ...
        if (!isNaN(oldDeadline.getTime())) {
            ...
            /*
             * Keep advancing the deadline
             * until it belongs to the
            ...
            if (deadlineDisplay) {
                ...
                const time =
                    newDeadline.toLocaleTimeString(
                        [],
                        {
                            hour: "2-digit",
                            minute: "2-digit"
...
function saveData() {
   ...
   // Save mission penalty states
document.querySelectorAll("#mission-list li").forEach(li => {

    if (li.dataset.deducted === "true") {
        li.setAttribute("data-deducted", "true");
    } else {
        li.removeAttribute("data-deducted");
    }

    if (li.dataset.hardcorePunished === "true") {
        li.setAttribute(
            "data-hardcore-punished",
            "true"
        );
    } else {
        li.removeAttribute(
...
async function resetData() {
    ...
    try {
        ...
        lastImprovementDate = getTodayKey();
...
/* =========================================================
   LIVE RECURRING MISSION REFRESH
   ========================================================= */
...
setInterval(() => {
    ...
    try {
        ...
        console.error(
            "Recurring mission refresh failed:",
            error
        );
